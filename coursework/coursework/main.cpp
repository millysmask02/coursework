		...
		if ((level > 0) && (level < 4) && (!win) && (gameTimeLimitation != gameTime)) {
			...
			for (int i = 0; i < sizeLevel; i++) {
				for (int j = 0; j < sizeLevel; j++) {
					...
					sprSquareLevel1.setPosition(float(35 + cellSize * i), float(35 + cellSize * j));
			...
			for (int i = 1; i <= sizeLevel; i++) {
				for (int j = 1; j <= sizeLevel; j++) {
					...
					sprLapa.setPosition(float(p.x), float(p.y));
					sprLapa.move(float(offset.x - cellSize), float(offset.y - cellSize));
					window.draw(sprLapa);
				}
			}
